...
const AddNewPaletteModal = (props) => {
  const COLORS = [
    ...
    { colorName: "YellowGreen", hexCode: "#9ACD23" },
  ];
  const [paletteName, setPaletteName] = useState("");
  const [selectedColors, setSelectedColors] = useState([]);
  const handleUpdate = (item, newValue) => {
    if (newValue) {
  ...
  const handleSubmit = () => {
    if (!paletteName) {
      ...
      props.setPalettes((current) => [
        { paletteName: paletteName, colors: selectedColors },
        ...current,
      ]);
      props.onPress();
    }
  ...
  return (
    <View style={styles.container}>
      <View style={styles.topContainer}>
        ...
        <MaterialIcons name="close" size={28} onPress={props.onPress} />
      </View>
      <Text style={{ fontWeight: "900", fontSize: 20, marginVertical: 10 }}>
        Enter Palette's Name
      </Text>
      <TextInput
        style={{
          padding: 10,
